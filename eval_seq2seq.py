...
def stem_word_list(word_list, lang):
    ...
    """
        ...
        if lang == 'latvian':
            ...
            return [stemmer_sl.lemmatize(w.strip()) for w in word_list]
    """
...
def cross_eval(detected_keywords, gold_standard_keywords, remove_docs_with_no_kw=True, langs = []):
    all_p_et_5 = []
    all_r_et_5 = []
    all_p_et_10 = []
    all_r_et_10 = []
    ...
    no_kw = 0
    all = 0
    if not type(langs) == list:
        langs = langs.flatten().cpu().detach().tolist()
    for preds, true, lang in zip(detected_keywords, gold_standard_keywords, langs):
        preds = [x.split() for x in preds]
